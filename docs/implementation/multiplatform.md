...
### Key Configuration File: `build.gradle.kts`

#### Target Definitions

```kotlin
kotlin {
    // Android: Standard Android Gradle Plugin target
    androidTarget { 
        compilerOptions { jvmTarget.set(JvmTarget.JVM_11) } 
    }

    // iOS: Two targets for device and simulator
    ...
    // Desktop: Standard JVM target
    jvm()

    // Web: JavaScript browser target
    js {
        browser {
            testTask {
                // Skip JS tests unless Chrome is configured
    ...
    // WebAssembly: Experimental Wasm target
...
sourceSets {
    ...
    androidMain.dependencies {
        ...
        implementation(uiLibs.kalendar.kit)  // Calendar integration
    }

    // iOS only
    iosMain.dependencies {
        implementation(uiLibs.ktor.client.darwin)  // Native HTTP
    ...
    // Web only
    jsMain.dependencies {
        implementation(uiLibs.ktor.client.js)  // fetch-based HTTP
    }
}
```
