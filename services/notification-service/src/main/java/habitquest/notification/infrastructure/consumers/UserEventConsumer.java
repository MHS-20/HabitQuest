...
public class UserEventConsumer implements EventConsumer {
    ...
    public UserEventConsumer(UserEmailRepository userEmailRepository, NotificationService notificationService) {
    ...
    public Consumer<UserRegisteredMessage> userRegistered() {
        return message -> {
            logger().info("Received UserRegistered: userId={}, email={}", message.userId(), message.email());
