package compose.project.demo.contexts.quest.presentation.screen
...
import compose.project.demo.contexts.quest.infrastructure.repository.CreateQuestResult
import compose.project.demo.contexts.quest.infrastructure.repository.JoinQuestResult
import compose.project.demo.contexts.quest.infrastructure.repository.QuestData
import compose.project.demo.contexts.quest.infrastructure.repository.QuestListResult
import compose.project.demo.contexts.quest.infrastructure.repository.QuestProgressData
import compose.project.demo.contexts.quest.infrastructure.repository.QuestProgressListResult
import compose.project.demo.contexts.quest.infrastructure.repository.QuestRepository
import kotlinx.coroutines.launch

@Composable
fun QuestScreen(token: String, avatarState: AvatarUiState, progressRefreshTick: Int = 0) {
  ...
  Column(
    ...
    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
      ...
      TextButton(onClick = { scope.launch { loadProgressForAvatar() } }) { Text("Refresh") }
    }
    if (progress.isEmpty()) {
      ...
      LazyColumn(
        ...
        items(progress, key = { it.questId }) { item -> QuestProgressRow(item) }
      }
    }
      ...
      AlertDialog(
        ...
        text = {
          Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Text(
              "${selectedDurationDays.toInt()} days",
            ...
            val selectedDurationPreviewDays =
              resolveQuestDurationDays(days = selectedDurationDays.toInt())
            Text(
        ...
        confirmButton = {
          TextButton(
            onClick = {
              ...
              val durationDays = resolveQuestDurationDays(days = selectedDurationDays.toInt())
              ...
              scope.launch {
                ...
                when (
                  val result =
                    repository.createQuestWithDetails(
                      token = token,
                      name = questName.trim(),
...
private fun resolveQuestDurationDays(days: Int): Int? {
...
private fun QuestRow(quest: QuestData, canJoin: Boolean, onJoin: () -> Unit) {
  Card(
    ...
    Column(modifier = Modifier.padding(12.dp)) {
      ...
      Spacer(Modifier.height(8.dp))
      Button(onClick = onJoin, enabled = canJoin, modifier = Modifier.fillMaxWidth()) {
        Text("Join quest")
      }
    }
  }
