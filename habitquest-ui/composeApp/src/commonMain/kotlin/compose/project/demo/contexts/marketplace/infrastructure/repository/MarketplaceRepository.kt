...
class MarketplaceRepository : MarketplaceGateway {
  ...
  override suspend fun ensureMarketplace(token: String, avatarId: String): MarketplaceLoadResult {
    val response =
      runCatching {
  ...
  override suspend fun buyItem(
    ...
    return when (response.status) {
      ...
      HttpStatusCode.OK -> MarketplaceBuyResult.Success
  ...
  override suspend fun sellItem(
    ...
    return when (response.status) {
      HttpStatusCode.NoContent,
      HttpStatusCode.OK -> MarketplaceSellResult.Success
