package compose.project.demo.contexts.dashboard.presentation.screen
...
import compose.project.demo.contexts.avatar.application.SpendSkillPointUseCase
...
import compose.project.demo.contexts.avatar.presentation.component.AvatarCard
import compose.project.demo.contexts.habits.application.CreateHabitUseCase
...
fun DashboardScreen(
    ...
    val createHabitUseCase = remember { CreateHabitUseCase(habitRepository) }
    val spendSkillPointUseCase = remember { SpendSkillPointUseCase(avatarRepository) }
    ...
    var historyState by remember {
        mutableStateOf<DashboardHistoryUiState>(DashboardHistoryUiState.Loading)
    }
    var isSpendingSkillPoint by remember { mutableStateOf(false) }
    var skillPointMessage by remember { mutableStateOf<String?>(null) }
    ...
    LaunchedEffect(token, avatarState) {
        ...
        historyState =
            when {
                token.isBlank() -> {
                    DashboardHistoryUiState.Error("Invalid session")
                }

                avatar == null -> {
                    DashboardHistoryUiState.Error("Avatar not available")
                }
            ...
            Column(
                ...
                when (val state = avatarState) {
                    ...
                    is AvatarUiState.Ready -> {
                        AvatarCard(avatar = state.avatar, onRefreshStats = onRefreshStats)
                        ...
                        Row(
                            ...
                            Button(
                                onClick = {
                                    scope.launch {
                                        isSpendingSkillPoint = true
                                        skillPointMessage = null
                                        when (
                                            val result =
                                                spendSkillPointUseCase(token, state.avatar.id, AvatarStatType.STRENGTH)
                                        ) {
                                            AvatarStatIncrementResult.Success -> {
                                                skillPointMessage = "Strength +1"
                                                onRefreshStats()
                                            }

                                            is AvatarStatIncrementResult.Error -> {
                                                skillPointMessage = result.message
                                            }
                                        }
                                        isSpendingSkillPoint = false
                                    }
                                },
                                enabled = !isSpendingSkillPoint,
                                modifier = Modifier.weight(1f),
                            ) {
                                if (isSpendingSkillPoint) {
                            ...
                            Button(
                                onClick = {
                                    scope.launch {
                                        ...
                                        when (
                                            val result =
                                                spendSkillPointUseCase(token, state.avatar.id, AvatarStatType.DEFENSE)
                                        ) {
                                            AvatarStatIncrementResult.Success -> {
                                                skillPointMessage = "Defense +1"
                                                onRefreshStats()
                                            }
                            ...
                            Button(
                                onClick = {
                                    scope.launch {
                                        ...
                                        when (
                                            val result =
                                                spendSkillPointUseCase(token, state.avatar.id, AvatarStatType.INTELLIGENCE)
                                        ) {
                                            AvatarStatIncrementResult.Success -> {
                                                skillPointMessage = "Intelligence +1"
                                                onRefreshStats()
                                            }
                ...
                if (showCreateHabitDialog) {
                    AlertDialog(
                        ...
                        dismissButton = {
                            TextButton(onClick = { showCreateHabitDialog = false }) { Text("Cancel") }
                        },
                    )
                }
                ...
                if (habitMessage != null) {
                    ...
                    Text(
                        ...
                        color =
                            if (habitMessage?.startsWith("Habit created") == true) {
                                MaterialTheme.colorScheme.primary
                            } else {
                                MaterialTheme.colorScheme.error
                            },
                ...
                when (val state = historyState) {
                    DashboardHistoryUiState.Loading -> {
                        Text("Loading history...")
                    }

                    is DashboardHistoryUiState.Error -> {
                        Text(text = state.message, color = MaterialTheme.colorScheme.error)
                    }
